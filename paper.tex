...
We therefore split GEMM2 and reduction: GEMM2 writes non-atomic \texttt{expert\_out} (bf16 where enabled), then a token-centric reduce loads 8 contributions per token. For large $T$, 2D tiling cuts the reduce grid $8\times$.
...
\midrule
$1$ & singleton & specialized & GEMM2 \\
$7$--80 & small/mid traces & $\sim$40--70$\times$ & GEMM1 \\
$901$ & isolated & session-dep. & transition \\
...
\textbf{Large hybrid $T$ (11{,}948--14{,}107):} GEMM2 becomes the main bottleneck, while CuTe improves the regular grouped-GEMM core.
