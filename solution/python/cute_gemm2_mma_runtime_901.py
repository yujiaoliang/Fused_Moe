...
_CUTE_GEMM2_TARGET_T = 901
_CUTE_GEMM2_BLOCK_M = 64
...
def _load_gemm2_module():
    ...
    if _CUTE_GEMM2_ERROR is not None:
        raise RuntimeError(f"cached CuTe GEMM2 MMA T=901 failure: {_CUTE_GEMM2_ERROR!r}")

    try:
        _CUTE_GEMM2_MOD = _load_local_module("hybrid_cute_gemm2_mma_t901", "cute_gemm2_mma_t901.py")
        return _CUTE_GEMM2_MOD
...
def run_cute_gemm2_mma(
    ...
    if not should_use_cute_gemm2_mma(int(total_tokens), int(block_m)):
        raise RuntimeError("CuTe GEMM2 MMA T=901 runtime is not selected for this workload")
