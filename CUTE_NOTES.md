...
| Chunked 16-expert dispatch (remap) | `block_m < 128` 拦截，CuTe 未实际执行。即使验证通过，天花板仅 +2~3× mean (§9) |
...
## 8. [2026-04-08] Main Sync + 全量验证

### 8.1 合并操作

将 `main` 分支最新 9 个 commit merge 到 `CuTe-dev`，冲突文件全部取 `main` 版本（`kernel.py`, `log.md`, `profiling_notes.md`, `solution.json`）。

合并的 main 提交包括：
- `076dd81` refactor: remove ~250 lines of dead remapped GEMM1 code
- `d8b380f` feat: GEMM2 autotune expansion (AB-test mean +2.1%)
- `c19336d` feat: fused routing + histogram
- `16d7ffb` fix: disable remapped GEMM1 (dead code)
- `c7aa617` doc: block_m 128 / block_k 256 experiments
- 以及其他 doc/cleanup commits

**注意**：merge 后 `kernel.py` 完全取自 main，不再包含 CuTe 集成代码。CuTe 相关代码仅存在于 `cute_fused_gemm.py` 和 `cute_kernel.py`（保留为技术存档）。

### 8.2 Full 19/19 Benchmark 验证 (Modal B200, 2026-04-08)
...
- **Mean**: ~42.5x
- **Peak**: 52.43x (T=1)
- **大 T**: 10.54x / 12.15x（纯 Triton fallback，与 main 一致）

### 8.3 验证结论

1. **19/19 PASSED** — merge 后没有引入正确性 bug
2. **CuTe 路径依然零执行** — `cute_fused_gemm.py` 和 `cute_kernel.py` 存在但从未被调用（`kernel.py` 已是 main 版本，不含 CuTe dispatch 入口）
3. **性能与 main 分支一致** — 所有性能 100% 来自 Triton kernels
4. **CuTe-dev 分支正式进入维护模式** — 不再做新的 CuTe 优化，代码保留作为技术博客素材和未来参考

### 8.4 CuTe 分支最终状态

```
CuTe-dev 分支状态:
├── kernel.py              ← 来自 main (纯 Triton，含 GEMM2 autotune 等最新优化)
├── cute_fused_gemm.py     ← CuTe host dispatch (保留，从未被调用)
├── cute_kernel.py         ← CuTe DSL kernel 类 (保留，从未被调用)
├── CUTE_NOTES.md          ← 本文档 (完整开发记录)
└── 其他文件               ← 与 main 同步
```

---

## 9. [2026-04-08] Remap 实验：Chunked 16-Expert Dispatch

### 9.1 实验动机

在 Section 3 的 post-mortem 中，我们将 `num_g > 16` crash 归因于 CuTe DSL/PTS 内部 bug，并基于 5 个实验判定其"无法绕过"。但事后复审发现，此前的 Batch Split (16+16) 实验中，Batch 1（experts 0-15）成功而 Batch 2（experts 16-31）crash 的现象，**也可以用 tensormap offset 溢出来解释**——即 PTS 只为 `group_idx ∈ [0, 15]` 正确生成了 tensormap，超出后地址计算越界。

为获得 100% 确定性（排除"可以修但我们没试到"的遗憾），设计了一个限时 2 小时的 remap 实验。

### 9.2 实验方案

**核心思路：** 将 32 个 active experts 拆分为 2 个 chunk（每个 ≤ 16），每个 chunk 独立构建 `ptrs/shapes/strides` 表并独立调用 CuTe kernel。每个 chunk 内的 `local_g` 索引从 0 开始，确保 PTS 只看到 `group_idx ∈ [0, 15]`。

**代码变更 (`cute_fused_gemm.py`)：**

```python
# 移除 num_g > 16 的 early exit guard
# 改为 chunked dispatch:
for chunk_start in range(0, num_experts, 16):
    chunk_end = min(chunk_start + 16, num_experts)
    
    chunk_num_g = sum(1 for e in range(chunk_start, chunk_end)
                      if e_starts[e+1] - e_starts[e] > 0)
    if chunk_num_g == 0:
        continue
    
    # 为 THIS chunk 构建独立的 ptrs/shapes/strides
    # local_g 从 0 开始 → PTS 只看到 group_idx ∈ [0, chunk_num_g-1]
    _dispatch_bufs = _get_dispatch_bufs(chunk_num_g, H, KBLKS_32, N_DIM)
    ...
    # 独立 JIT 编译 + launch
    _CUTE_COMPILED(... chunk_num_g ...)
```
...
19/19 PASSED
  ...
  5e8dc11c (T=14107): 5.121ms | 10.57x  ← 与纯 Triton 一致
  58a34f27 (T=11948): 3.606ms | 12.26x  ← 与纯 Triton 一致
  (其余 17 workload 与 main 一致)
```

**关键发现：CuTe 路径从未被调用。** 

原因：`kernel.py` 在 main sync 后，大 T workload 的 `block_m` 不再是 128。main 的 Triton autotuning 已经将这些 workload 配置为更优的 `block_m` 值，而 `cute_fused_gemm.py` 的 `block_m < 128` guard（L201-203）直接拦截了所有调用。

### 9.4 为什么不继续修改 kernel.py 来强制 block_m=128？

| 考量 | 分析 |
|------|------|
| **天花板** | 即使 CuTe 完美工作，只影响 2/19 workload（T=11948, T=14107）|
| **瓶颈错位** | 这 2 个 workload 的瓶颈是 GEMM2 (49-50%)，不是 GEMM1 (40%) |
| **最好情况** | GEMM1 提速 4× → 总时间 -30% → 整体 mean +2~3× |
| **维护代价** | 需要在 kernel.py 维护独立的 CuTe dispatch 路径、JIT 编译缓存、fallback 逻辑 |
| **ROI 对比** | Direction 1 (per-expert packing) 影响 17/19 workload，可消灭 75-94% padding waste |

**结论：ROI 差距达一个数量级。根据 2 小时止损策略，立即停止。**

### 9.5 最终确定性评估

| 维度 | 确定性 | 更新 |
|------|--------|------|
| 当前 0/19 执行 CuTe | ✅ 100% | 不变 |
| `block_m >= 128` 硬约束 | ✅ 100% | 不变 |
| `num_g > 16` 完全无法绕过 | ⚠️ **未验证** | chunked dispatch 代码已写入但因 block_m 拦截从未执行 |
| 即使修好收益也有限 | ✅ 95% | 天花板分析已确认：最好 +2~3× mean |
| Direction 1 ROI 更高 | ✅ 99% | 不变 |

**技术遗憾**：`num_g > 16` 的 remap workaround 是否能绕过 crash 这个问题**没有得到最终验证**（被更上游的 `block_m < 128` guard 拦截了）。但这个 unknown 对决策没有影响——即使答案是 yes，CuTe 的天花板（+2~3× mean）也不值得投入。

### 9.6 CuTe 章节永久关闭

**此实验标志着 CuTe 优化路径的正式、永久关闭。**

- `cute_fused_gemm.py` 保留 chunked dispatch 代码作为技术存档
- `cute_kernel.py` 保留 kernel 类定义作为技术存档
- 后续开发全部在 `main` 分支进行，聚焦 Direction 1 (Per-Expert BLOCK_M Packing)
- 不再投入任何时间调查 CuTe 相关问题
