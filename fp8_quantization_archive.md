# 🗄️ 归档日志：Dynamic FP8 Quantization in Triton on Blackwell (B200)

> **【历史档案馆】(Archived as of Phase 8 - 2026-03-27)**
> 
> *这是一份关于我们在 Triton 环境下手工捏造 Blackwell PTX 指令体系（绕过 Triton 编译器缺陷实现 FP8 动态量化）的极客死磕记录。*
> 
> **为什么最终归档了这条战线？**
> 1. **精度死区**：正如本日志后半部分剖析的那样，SwiGLU 的动态激活范围存在极端重尾分布（Outliers）。即便我们后来通过极细粒度的 Block-Scale 和混合 `top-k` 稀疏 Residual 来做数值补偿，仍然难以在不损失大量运行速度的前提下完美压回误差容限（`matched_ratio > 0.95`）。
> 2. **更暴力的解法横空出世**：在主分支的 Phase 7 和 Phase 8 中，我们转而从**维度调度**和**L2 缓存控制**上寻找突破口。通过提出惊艳的 **`Column-Major` (列排布)** 并发预读与 **2D Tiled Map-Reduce (`evict_last`)** 算法，我们在纯血量标准的 `FP32` 数据链路下直接打穿了 B200 的 HBM 带宽墙，拿到了 `4.96ms` (T=8192) 的逆天统治级成绩。
> 3. **降维打击**：在纯洁的 FP32 下拿到的性能，甚至远超这些为了弥补 FP8 寄存器洗牌和打包成本而做出的各种复杂拆解设计。在绝对的硬件吞吐和顶级计算调度机制面前，强上 FP8 反而成了一种吃力不讨好的性能累赘。
> 
> 这份日志如今作为最详尽的 Blackwell 底层指令级探星实录被**永久封存**，用来证明我们在彻底否定一条路之前，到底做过怎样极致而绝望的算法挣扎。
> 
